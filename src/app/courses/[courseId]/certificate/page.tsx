...
export default function CourseCertificatePage() {
  ...
  // No default URL; certificate is generated by backend
  ...
  const handleGenerateCertificate = async () => {
    ...
    try {
      ...
      // If feedback exists, request certificate generation from backend
      setGenerating(true);
      setError('');
      ...
      const certRes = await fetch(buildApiUrl('certificates/generate'), {
        ...
        body: JSON.stringify({ userId: user.id, courseId: String(params.courseId) }),
      });

      if (!certRes.ok) {
        const errorText = await certRes.text().catch(() => '');
        throw new Error(errorText || 'Failed to generate certificate.');
      }

      const certData = await certRes.json();
      const urlFromApi = certData?.certificateUrl || certData?.url;
      if (!urlFromApi) {
        throw new Error('Certificate URL missing in response.');
      }
      setCertificateUrl(urlFromApi);
      
    } catch (err: any) {
      // Surface error without falling back to a default URL
